...
def upsert_prefix_into_path_stem(
    filename: str,
    prefix: str,
...
def upsert_suffix_into_path_stem(
    filename: str,
    suffix: str,
    validator: Optional[Callable[[str], bool]] = None,
    sep: str = "_",
...
def list_items_in_dir(
    ...
    if not recursive:
        ...
        all_names = []
        if full_path:
            for root, dirnames, filenames in os.walk(path_ex, followlinks=followlinks):
                all_names.extend(os.path.join(root, name) for name in sorted(dirnames))
                all_names.extend(os.path.join(root, name) for name in sorted(filenames))
        else:
            for root, dirnames, filenames in os.walk(path_ex, followlinks=followlinks):
                relative_path = os.path.relpath(root, path_ex)
                if relative_path == '.':
                    all_names.extend(sorted(dirnames))
                    all_names.extend(sorted(filenames))
                else:
                    all_names.extend(os.path.join(relative_path, name) for name in sorted(dirnames))
                    all_names.extend(os.path.join(relative_path, name) for name in sorted(filenames))
        return all_names


def list_dirs_in_dir(
    path: Optional[str] = None,
    recursive: bool = False,
    full_path: bool = True,
    followlinks: bool = False,
    ...
    if not recursive:
        ...
        all_names = []
        if full_path:
            for root, dirnames, _ in os.walk(path_ex, followlinks=followlinks):
                all_names.extend(os.path.join(root, name) for name in sorted(dirnames))
        else:
            for root, dirnames, _ in os.walk(path_ex, followlinks=followlinks):
                relative_path = os.path.relpath(root, path_ex)
...
def list_files_in_dir(
    ...
    if not recursive:
        ...
        all_names = []
        if full_path:
            for root, _, filenames in os.walk(path_ex, followlinks=followlinks):
                all_names.extend(os.path.join(root, name) for name in sorted(filenames))
        else:
            for root, _, filenames in os.walk(path_ex, followlinks=followlinks):
                relative_path = os.path.relpath(root, path_ex)
...
def copy_file(
    src: str,
    dst_dir: str,
    action_if_exist: Optional[Literal["ignore", "rename"]] = "rename",
) -> str:
    """
    Args:
